...
function myPreload() {
    ...
    // create a second ball: load a 2-frame 32x32px spritesheet for 2nd ball
    this.load.spritesheet(
      'ball2_spritesheet',
      'img/orange_fireball.png',
      {
        frameWidth: 32,
        frameHeight: 32
      }
...
function myCreate() {
    ...
    ball2 = this.physics.add.sprite( 900, 500, 'ball2_spritesheet' );
    ball2.setCollideWorldBounds( false );
    ball2.setBounce(1);
    let ball_anim_frames = this.anims.generateFrameNumbers(
        'ball2_spritesheet',
        {
        start: 0,
        end: 1
    ...
    let ball_animation = this.anims.create(
        {
        key: 'flaming',
        frames: ball_anim_frames,
        frameRate: 15,
        repeat: -1,
        yoyo: false
        }
    );
    ball2.anims.play('flaming');
    this.tweens.add(
        {
            targets: ball2,
            duration: 1000,
